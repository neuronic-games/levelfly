...
class CourseGrade < ActiveRecord::Base
  ...
  def self.load_grade(profile_id, course_id, school_id)
    ...
    [course_grades, outcome_grades]
  end

  def self.load_outcomes(profile_id, course_id, outcome_id, school_id)
    outcome_point = CourseGrade.where('school_id = ? and profile_id = ? and course_id = ? and outcome_id = ?',
                                      school_id, profile_id, course_id, outcome_id).first
  ...
  def self.load_notes(profile_id, course_id, school_id)
    course_notes = CourseGrade.where('school_id = ? and profile_id = ? and course_id = ? and outcome_id IS NULL',
                                     school_id, profile_id, course_id).first
    if course_notes.nil?
      ''
    else
      course_notes.notes
  ...
  def self.update_outcome_average(outcome_id, task_id, course_id)
    ...
    participant_profile_ids.each do |profile_id|
      ...
      next unless course_grade and !course_grade.nil?

      outcome_grade = OutcomeGrade.where([
                                           'course_id = ? and profile_id =? and  outcome_id = ? and grade is not null and task_id in (?)', course_id, profile_id, outcome_id, tasks
                                         ]).map(&:grade)
