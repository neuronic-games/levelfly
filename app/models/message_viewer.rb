class MessageViewer < ActiveRecord::Base
  ...
  def self.add(profile_id, message_id, parent_type, parent_id)
    ...
    if %w[C G F].include?(parent_type)
      @viewers = Profile.where([
                                 "participants.target_id = ? AND participants.target_type in ('Course','Group') AND participants.profile_type IN ('M', 'P', 'S')", parent_id
                               ])
      ...
      @viewers = Profile.where([
                                 "participants.target_id = ? AND participants.target_type = 'User' AND profile_type = 'F'", profile_id
                               ])
                        .includes([:participants])
                        .joins([:participants])
      MessageViewer.create(message_id: message_id, poster_profile_id: profile_id,
