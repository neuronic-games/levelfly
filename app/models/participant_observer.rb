  ...
  def after_save(participant)
    if participant.profile_type == "S"
      message_ids = MessageViewer.where(["viewer_profile_id = ?", participant.target.owner.id])
        .select("message_id")
        .collect(&:message_id)
      messages = Message.where(["parent_id = ? and parent_type in ('C','F','G') and archived = ? and id in (?)",participant.target.id, false, message_ids])
      messages.each do |message|
        message_viewer = MessageViewer.where(["viewer_profile_id = ? and poster_profile_id = ? and message_id = ?",participant.profile_id, message.profile_id, message.id]).first
