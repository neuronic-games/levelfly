...
class Badge < ActiveRecord::Base
  ...
  def self.load_all_badges(profile)
    ...
    @badges = Badge.where(
      'school_id = ? and (creator_profile_id = ? or creator_profile_id IS NULL) and badge_image_id not in (?) and archived != true', profile.school_id, profile.id, gold_image_id
  ...
  def self.gold_outcome_badge(outcome_name, badge_creator)
    @badge = find_by_name("Gold Medal in #{outcome_name}")
  ...
  def self.gold_badge_image
    ...
    gold_badge ||= BadgeImage.create(image_file_name: Badge.gold_badge,
  ...
  def self.find_create_game_badge(game_id, name, descr = nil, badge_image_id = nil)
    ...
    if badge.nil?
      ...
      badge.available_badge_image_id = badge.badge_image_id unless badge_image_id.nil?
    end
  ...
  def image_url
    badge_image ? badge_image.image_file_path : BadgeImage.blank
  end

  def available_image_url
    return BadgeImage.available_image_by_badge(available_badge_image_id) if available_badge_image_id.present?
    if badge_image and badge_image.image_file_size.nil?
    ...
    try(:badge_image).try(:image).try(:url)
  end
end
