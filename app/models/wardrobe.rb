class Wardrobe < ActiveRecord::Base
  ...
  def self.add(wardrobe_name, level_0_name, level_1_name, name, item_type, image_file, sort_order = nil, new_name = nil)
    ...
    if wardrobe.nil?
      wardrobe = Wardrobe.create(name: wardrobe_name, visible_level: 1, available_level: 1,
                                 available_date: Date.today, visible_date: Date.today)
    ...
    if exists
      ...
      if sort_order.nil?
        sort_order = WardrobeItem.maximum(:sort_order,
                                          conditions: ['parent_item_id = ? and depth = 1',
                                                       level_0.id]) + 1
      end
      item = WardrobeItem.create(wardrobe: wardrobe, parent_item: level_1, name: new_name || name,
                                 item_type: item_type, image_file: image_file, sort_order: sort_order, depth: 2)
