class TaskGrade < ActiveRecord::Base
  ...
  def self.task_grades(school_id, course_id, task_id, profile_id, task_grade, grade)
    @grade = TaskGrade.where('school_id = ? and course_id = ? and task_id =? and profile_id = ? ', school_id, course_id,
                             task_id, profile_id).first
    CourseGrade.save_grade(profile_id, grade, course_id, school_id)
    if @grade.nil?
      task_grade_save(school_id, course_id, task_id, profile_id, task_grade)
    else
      task_grade_update(task_grade, @grade)
    end
  end
  ...
  def self.load_task_grade(school_id, course_id, task_id, profile_id)
    task_grade = TaskGrade.where('school_id = ? and course_id = ? and task_id = ? and profile_id = ?', school_id,
                                 course_id, task_id, profile_id).first
    if task_grade.nil?
      nil
    else
      task_grade.grade
  ...
  def self.grade_average(school_id, course_id, profile_id, task_id = nil, task_grade = nil)
    ...
    if task_id
      previous_task_grade = TaskGrade.where('school_id = ? and course_id = ? and task_id =? and profile_id = ? ',
                                            school_id, course_id, task_id, profile_id).first
    end
    ...
    tasks.each do |task|
      ...
      tg = TaskGrade.where(['school_id = ? and course_id = ? and task_id =? and profile_id = ?', school_id, course_id,
                            task.id, profile_id]).first
  ...
  def self.sort_tasks_grade(profile_id, course_id)
    TaskGrade.where([
                      "task_grades.course_id = ? and task_grades.profile_id = ? and task_grades.grade IS NOT NULL and tasks.archived = 'false'", course_id, profile_id
                    ])
             .includes([:task])
             .joins([:task])
             .map(&:task_id)
