class CourseController < ApplicationController
  ...
  def show
    ...
    @groups = Group.find(:all, :conditions=>["course_id = ?",@course.id])
     message_ids = MessageViewer.find(:all, :select => "message_id", :conditions =>["viewer_profile_id = ?", @profile.id], limit: 200).collect(&:message_id)
    if params[:section_type]=="C"
      ...
      message_ids = MessageViewer.find(:all, :select => "message_id", limit: 200).collect(&:message_id) if @member.nil?
  ...
  def show_course
    ...
    if params[:value] && !params[:value].nil?
      if (params[:section_type] == "G" && params[:value] == "1")
        render :partial => "/group/group_wall", :locals => {privilege: params[:privilege].present?, game: @course.game} 
