class TaskController < ApplicationController
  ...
  def index
    ...
    if @profile
      @courses = Course.where([
                                'participants.profile_id = ? AND parent_type = ? AND participants.profile_type != ? AND courses.archived = ? AND courses.removed = ? ', @profile.id, Course.parent_type_course, Course.profile_type_pending, false, false
                              ])
  ...
  def show
    ...
    all_tasks = []
    all_tasks = Task.filter_by(@course.owner.id, @course.id, '').collect(&:points) if @course.owner
    ...
    @people = Participant.where([
                                  "participants.target_id = ? AND participants.target_type='Course' AND participants.profile_type = 'S' AND users.status != 'D'", @task.course_id
                                ])
                         .includes([profile: :user])
                         .order('profiles.full_name')
    @task_members = TaskParticipant.where([
                                            "task_participants.task_id = ? AND task_participants.profile_type = 'M' AND users.status != 'D'", @task.id
                                          ])
                                   .includes([profile: :user])
                                   .order('profiles.full_name')
      ...
      if params[:people_id] && !params[:people_id].empty?
        if params[:people_id] == 'all_people'
          ...
          peoples_array.each do |p_id|
            ...
            task_participant = TaskParticipant.where(["task_id = ? AND profile_type='M' AND profile_id = ?",
                                                      @task.id, p_id]).first
            next if task_participant
  ...
  def view_task
    @profile = current_profile
    @courses = Course.where([
                              'participants.profile_id = ? AND parent_type = ? AND participants.profile_type != ? AND courses.archived = ?', @profile.id, Course.parent_type_course, Course.profile_type_pending, false
                            ])
                     .distinct
                     .includes([:participants])
                     .order('courses.name')
