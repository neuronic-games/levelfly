class ProfileController < ApplicationController
  ...
  def index
    ...
    if params[:search_text]
      ...
      @profiles = Profile.where([
                                  "school_id = ? and (lower(full_name) LIKE ? OR lower(users.email) LIKE ?) and user_id is not null and users.status != 'D'", @profile.school_id, search_text.downcase, search_text.downcase
                                ])
                         .includes([:user])
                         .order('full_name')
                         .joins([:user])
  ...
  def save
    ...
    if @avatar.save
      ...
      # Create a temporary attachment for avatar storage
      attachment = Attachment.upload_base64(@profile.school_id, file_name, Base64.decode64(params[:avatar_img]))
      if attachment && attachment.save
        @profile.image_file_name = url_for(attachment.resource)
        @profile.save
      end
    end
    publish_profile(@profile)
  ...
  def user_profile
    if params[:profile_id].blank?
      ...
      @badge = Badge.badge_count(@profile.id)
      notes = Note.where(["profile_id = ? and about_object_id = ? and about_object_type = 'Profile'",
                          user_session[:profile_id], @profile.id]).first
      @notes = notes.content unless notes.nil?
    end
  ...
  def edit_wardrobe
    @profile = if params[:profile_id].blank?
  ...
  def show_comments
    @profile = if params[:id] and !params[:id].nil?
                 Profile.find(params[:id])
               else
                 Profile.where(['user_id = ?', current_user.id]).first
               end
    @profile.all_comments = params[:show] == 'true'
    @profile.save
  ...
  def change_extended_logout_preference
    ...
    render json: { success: true } and return if profile.save
