class LeaderBoardController < ApplicationController
  ...
  def get_rows
    ...
    profiles_temp = if params[:show] and !params[:show]
  ...
  def show_user_profile
    if params[:course_id] && !params[:course_id].nil?
      @courses_list = Course.joins(participants: [{ profile: :avatar }]).where(
        "participants.target_type = 'Course' AND courses.id = ?", params[:course_id]
      ).select('profiles.full_name,avatars.level,avatars.points,courses.name,profiles.image_file_name')
    end
