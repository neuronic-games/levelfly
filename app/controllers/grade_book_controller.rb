...
class GradeBookController < ApplicationController
  ...
  def load_outcomes
    unless params[:course_id].blank?
      ...
      @participant = Participant.all(joins: [profile: :user],
                                     conditions: ["participants.target_id=? AND participants.profile_type = 'S' AND target_type = 'Course' AND users.status != 'D'", @course.id], select: ['profiles.full_name,participants.id,participants.profile_id'], order: 'full_name')
      unless @participant.nil?
        @participant.each do |p|
          outcomes_grade = []
