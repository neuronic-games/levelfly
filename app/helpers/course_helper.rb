module CourseHelper
  ...
  def all_course(parent_type)
    Course.where(['archived = ? and removed = ? and parent_type = ? and name is not null', false, false,
                  parent_type])
          .distinct
          .order('name')
  end
  ...
  def member_forum(member_id, course_id)
    if member_id
      ...
      course_participants = Participant.where(["target_id = ? AND target_type='Course' AND profile_type='S'",
                                               course.course_id])
      return true if (forum_participants.count == course_participants.count) and course_participants.present?
    end
    false
  end
end
